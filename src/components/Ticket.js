...
const Ticket = () => {
  ...
  const renderedSeats = ticket.passengers.map((passenger, i) => {
      return (
        <tr>
        ...
        <td data-label="Job">{`${ticket.status.toUpperCase()}/${Object.keys(ticket.seats)[0]} ${ticket.seats[Object.keys(ticket.seats)[0]][i]}`}</td>
      </tr>
      )
  })
